...
class Pi0Policy(BasePolicy):
    ...
    LeRobot의 Pi0 정책을 통합 인터페이스로 래핑한 클래스입니다.
    """
    ...
    def load_model(self, checkpoint_path: str = None) -> None:
        ...
        if checkpoint_path:
            # 로컬 체크포인트 로드
            self.policy = LeRobotPi0Policy.from_pretrained(checkpoint_path)
        elif pretrained_policy_name_or_path:
            # Hugging Face Hub에서 로드
            self.policy = LeRobotPi0Policy.from_pretrained(
    ...
    def get_policy_info(self) -> Dict[str, Any]:
        ...
        if self.policy:
            # Pi0 특정 정보 추가
            info.update({
                'vision_backbone': getattr(self.policy.config, 'vision_backbone', None),
                'pretrained_backbone_weights': getattr(self.policy.config, 'pretrained_backbone_weights', None),
                'replace_final_relu_by_sigmoid': getattr(self.policy.config, 'replace_final_relu_by_sigmoid', None)
            })
        return info
...
PolicyFactory.register_policy('pi0', Pi0Policy)