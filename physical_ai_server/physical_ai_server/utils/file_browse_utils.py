...
"""File browser utility functions for handling file system operations."""

import os
import datetime


def handle_get_path_action(current_path):
...
def handle_go_parent_action(current_path):
    ...
    if os.path.exists(parent_path) and os.path.isdir(parent_path):
        return {
            ...
            'selected_path': "",
            'items': get_directory_items(parent_path)
        }
    else:
        return {
            'success': False,
...
def handle_browse_action(current_path, target_name=None):
    """Handle browse action for directory or file selection."""
    current_path = current_path or os.path.expanduser("~")
    current_path = os.path.abspath(current_path)

    if target_name:
        return handle_target_selection(current_path, target_name)
    else:
        return handle_directory_browse(current_path)


def handle_target_selection(current_path, target_name):
    ...
    if os.path.exists(target_path):
        if os.path.isdir(target_path):
            ...
            return {
                ...
                'items': get_directory_items(target_path)
            }
        else:
            # Select file
            return {
...
def handle_directory_browse(current_path):
